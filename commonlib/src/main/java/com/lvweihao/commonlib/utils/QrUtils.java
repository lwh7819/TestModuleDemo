...
//                BinaryBitmap binaryBitmap = new BinaryBitmap(new GlobalHistogramBinarizer(source1));
    ...
    public static Bitmap createQRBitmap(Context context, String content, float width, float height, Bitmap logoBm) {
        try {
            if (content == null || "".equals(content)) {
                return null;
            }

            int widthPix = dip2px(context, width);
            int heightPix = dip2px(context, height);
